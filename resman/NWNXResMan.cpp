...
char* CNWNXResMan::DemandRes(CResStruct* cRes, char* resRef, NwnResType resType)
{
	...
	if (cRes->pResName)
	{
    Log(3, "o Request for internal resource = %s (type %X)\n", resRef, resType);
	...
	if (cRes->pClass)
	{
	  ...
	  void (*pServFunc)(CResStruct* cRes) = (void (*)(CResStruct* cRes)) pFunc;
	  pServFunc(cRes);
	}
...
/*
  if (size > MAXSIZE)
  {
    Log(1, "o Critical error - resource too large (>%ld bytes).\n", MAXSIZE);
    fclose (pTemp);
    return 0;
  }*/
...
void CNWNXResMan::WriteLogHeader()
{
	...
	Log (0, "(c) 2006-2007 by dumbo (dumbo@nm.ru) & virusman (virusman@virusman.ru)\n");
