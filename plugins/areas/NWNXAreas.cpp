...
char* CNWNXAreas::OnRequest(char* gameObject, char* Request, char* Parameters)
{
    ...
    } else if (strncmp(Request, "GET_AREA_LOADSCREEN", 19) == 0) {
        snprintf(Parameters, strlen(Parameters), "%d", NWNXGetAreaLoadscreen(gameObject, strtol(Parameters, (char **)NULL, 16)));
    } else if (strncmp(Request, "GET_AREA_PVP", 12) == 0) {
        snprintf(Parameters, strlen(Parameters), "%d", NWNXGetAreaPVP(gameObject, strtol(Parameters, (char **)NULL, 16)));
    ...
    } else if (strncmp(Request, "GET_TILE_ATLOC", 14) == 0) {
        snprintf(Parameters, strlen(Parameters), "%d", NWNXGetTileAtLocation(gameObject, Parameters));
