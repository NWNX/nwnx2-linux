...
void Func_SetMemorizedSpell (CGameObject *ob, char *value) {
    int i, sp_class, sp_level, sp_idx, sp_spell, sp_meta, sp_ready;
    CNWSCreature *cre;
    CNWSStats_Spell *sp;
    ...
    if (ob == NULL                                    ||
        ...
        cre->cre_stats == NULL                        ||
        sscanf(value, "%d %d %d %d %d %d", &sp_class, &sp_level, &sp_idx, &sp_spell, &sp_meta, &sp_ready) != 6 ||
